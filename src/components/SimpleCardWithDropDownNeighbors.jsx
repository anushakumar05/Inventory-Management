...
const SimpleCardWithDropDownNeighbors = ({items, img, header, mobileHeader, number, trend, description, icon}) => {

    const [selectedItem, setSelectedItem] = useState("");
    const [currentWeekCount, setCurrentWeekCount] = useState(0);
    const [previousWeekCount, setPreviousWeekCount] = useState(0);

    // Effect to set selectedItem after items have loaded
    useEffect(() => {
        if (items && items.length > 0) {
            setSelectedItem(items[0].itemName); // Set the first item in the list as the default
        }
    }, [items]);
    ...
    useEffect(() => {
        ...
        const countUniqueNeighborsPreviousWeek = (history) => {
          ...
          const prevWeekNeighbors = new Set(
            history
              .filter((entry) => {
                const entryDate = moment(entry.purchaseDate);
        ...
        if (selectedItem && items.length) {
          ...
          if (selectedItemData && selectedItemData.history) {
            const thisWeek = countUniqueNeighborsThisWeek(selectedItemData.history);
            const previousWeek = countUniqueNeighborsPreviousWeek(selectedItemData.history);
...
export default SimpleCardWithDropDownNeighbors